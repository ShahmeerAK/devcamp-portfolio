class BloggsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @blogg.save
        format.html { redirect_to blogg_url(@blogg), notice: "Blogg was successfully created." }
  ...
  def update
    respond_to do |format|
      if @blogg.update(blogg_params)
        format.html { redirect_to blogg_url(@blogg), notice: "Blogg was successfully updated." }
  ...
  private
    ...
    def blogg_params
      params.require(:blogg).permit(:name, :title, :content)
    end
end
